    ...
    if not request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if not request.user.is_authenticated:
                ...
                path = os.path.join(settings.MEDIA_ROOT, request.user.username, "CSV", "output",
            ...
            #
            # report_header = Csv.write_file(correct_category)
            #
...
def sendmail(request):
    ...
    mail = EmailMessage('test','test',from_email,[search_id],)
    mail.attach_file('MEDIA/champa.icmc/CSV/output/Difference of 10k.csv')
    mail.send()
